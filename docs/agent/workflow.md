...
> **状態: 確定(2026-07-19 人間承認・5人標準フロー)**
> 作成: 2026-07-05 / 5人構成更新: 2026-07-19 / 起案: Gen(玄)
> 対象範囲: **人間・Gen・Shino・Kai・Toki・Rinの標準フロー**。
...
10. 人間が実装を別途承認した場合だけ、8節の実装テーマ向けフローへ進む。
...
実装担当のTDD責任と混ぜず、分析、承認、骨組み、実装の順序を固定する。
...
4. 実装担当が、各項目のWhyを添えたテストコメントまたはテストの骨組みを作る。
   人間が骨組みをレビューするまで、実装本体を作らない。
5. 実装担当がRED → GREEN → REFACTORで実装し、テスト実行、自動化、回帰を所有する。
